...
def long_words(words):
    ...
    longer_than_4_char = [word for word in words if len(word) > 4]
    return longer_than_4_char
...
def n_long_words(words, n):
    ...
    longer_than_n_char = [word for word in words if len(word) > n]
    return longer_than_n_char
...
def find_letter_indices(words, letter):
    ...
    **DO NOT** use the `list.index()` method.
    ...
    `None`.)
    """
    letter_appears_at_index = []

    for word in words:  # good
        if letter in word:
            for index, let in enumerate(word):
                if let == letter:
                    letter_appears_at_index.append(index)
                    break
        else:
            letter_appears_at_index.append(None)

    return letter_appears_at_index
